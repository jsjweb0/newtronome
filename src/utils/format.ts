...
type DateValue = string | number | Date | FirestoreTimestampLike | null | undefined;

const isFirestoreTimestamp = (value: DateValue): value is FirestoreTimestampLike =>
  typeof value === 'object' &&
  value !== null &&
  'toDate' in value &&
...
export function formatDate(timestamp: DateValue, withTime = false): string {
  if (timestamp === null || timestamp === undefined || timestamp === '') return '';

  const date = isFirestoreTimestamp(timestamp) ? timestamp.toDate() : timestamp;

  return dayjs(date).format(withTime ? 'YYYY-MM-DD HH:mm' : 'YYYY-MM-DD');
}
