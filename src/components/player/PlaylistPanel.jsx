      ...
      <div className={`flex flex-col gap-3 h-full ${collapsed ? '' : 'hidden'}`}>
        ...
        >
            ...
            {isEditing ? (
              ...
              >
                <div className="flex items-center gap-x-2.5 mb-2.5">
                  ...
                  />
                  <label htmlFor="seletedAll" className="text-gray-700 dark:text-neutral-300">
                    전체(<b className="font-inter">{items.length}</b>)
                  </label>
                </div>
                <SortableContext
                ...
                <DragOverlay wrapperElement="ul">
                  {activeTrack ? (
                    <div className="shadow-lg rounded-lg backdrop-blur-xs">
                      <SortableItem id={activeTrack.id} track={activeTrack} hideListeners />
                    </div>
                  ) : null}
                </DragOverlay>
              </DndContext>
                  ...
                  tracks.map((track, idx) => (
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="block w-full col-start-2 text-sm text-textBase truncate group-hover:text-primary group-[.isActive]:text-primary">
                          {track.title}
                        </span>
                        <span className="col-start-2 row-start-2 block w-full text-[11px] text-textSub truncate group-hover:text-primary group-[.isActive]:text-primary">
                          {track.artist}
                        </span>
          ...
          {isEditing && selectedCount > 0 && (
            <div className="flex justify-between sticky bottom-0 w-full py-3 px-5 rounded-xl text-sm duration-350 animate-fade-slide-in border border-textThr bg-white dark:border-none dark:bg-textThr">
              <div className="text-gray-500 dark:text-neutral-400">
                <span className="text-primary font-semibold font-inter">{selectedCount}</span> 곡
                선택
              </div>
              <button
                type="button"
