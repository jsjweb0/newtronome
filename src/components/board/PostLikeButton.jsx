...
export default function PostLikeButton({ postId, boardType = 'free', showCount = true }) {
  ...
  useEffect(() => {
    const fetchLike = async () => {
      const likeDocRef = doc(db, boardType, postId.toString());
  ...
  const handleLike = async () => {
    ...
    if (!alreadyExists) {
      ...
      await updateDoc(likeDocRef, {
        ...
        likedUsers: newLiked ? arrayUnion(user.email) : arrayRemove(user.email),
      });
    }
  ...
  return (
    <div className="inline-block relative">
      ...
      >
        <Heart
          className={`shrink-0 size-4 transition-[fill] duration-300
