...
export default function ShareButton({
  ...
  useEffect(() => {
    ...
    if (kakao && !kakao.isInitialized()) {
      kakao.init('a1800562b80744b33ad9e4b5649d882a');
    }
  }, []);
  ...
  const shareToKakao = () => {
    ...
    const shareUrl =
      url ??
      `${window.location.origin}/board/pet/${post.desertionNo}`;


    kakao.Share.sendDefault({
      objectType: 'feed',
      content: {
        title: `[보호동물] ${post.kindNm ?? '정보 없음'}`,
        description: `${post.colorCd ?? '색상 정보 없음'} / ${post.careNm ?? '보호소 정보 없음'
          }`,
  ...
  return (
    <div className={`relative inline-flex ${className}`}>
      <button
        ...
        aria-expanded={showCopy && !closing}
