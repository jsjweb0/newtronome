...
- 비공식 SoundCloud API는 검색과 스트림 URL을 직접 제어할 수 있었지만, Client ID와 요청 규격 변경에 따라 재생이 중단되는 문제가 있었습니다. 기능 범위보다 안정성과 유지보수성을 우선해 공식 Widget으로 전환했고, 그 대신 Widget 이벤트와 렌더링 방식에 의존하는 제약을 받아들였습니다. 플레이리스트와 Likes 단일 재생은 `playbackMode`로 구분하고, 전환 전 재생 위치를 저장해 기존 흐름을 복원했습니다.
